...
#if defined(NDEBUG)
# error "Tanercoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5a0efed0b2ee72e85c7e762e9c595e0d391120c2f03214157bf2f71e7d8ee294");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tanercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tanercoin Signed Message:\n";
...
{
    // Tanercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Tanercoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 9 * 24 * 60 * 60; // Tanercoin: 3.5 days
static const int64 nTargetSpacing = 9 * 60; // Tanercoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tanercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tanercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd9dbde8ee29b6214a7111b66a6a5887318cd8d5f5391f4270ba0b715fa7c0316");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Bekir Taner Dincer, Professor at Mugla University since 2009, not yet died";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0416589b47e6f0c4e355c885108a43d81eff5bb5ddf9ca769ad5fed291acc945ffb6e552cea0b6a8842c3690df2770118aad674e2f5ba07dd3903a0e8593e42f48") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1520857311;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084541486;

        if (fTestNet)
        {
            block.nTime    = 1520857287;
            block.nNonce   = 386307064;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x77111742d40638fa5117b9352d7844780dbd92fd3c3a27f456c7dcff70cbe06b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Tanercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TanercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TanercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TanercoinMiner(CWallet *pwallet)
{
    printf("TanercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tanercoin-miner");
    ...
    try { loop {
        ...
        printf("Running TanercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TanercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TanercoinMiner, pwallet));
}
